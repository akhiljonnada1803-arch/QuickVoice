...
async def get_rag_context(agent_id: str, query: str, top_k: int = 5, tracer: Any = None, zero_pii: bool = False) -> str:
    """
    ...
    cm = tracer.span_rag(query=query, agent_id=agent_id, top_k=top_k, zero_pii=zero_pii) if tracer and getattr(tracer, "is_active", False) else contextlib.nullcontext({})
    with cm as rag_meta:
        if isinstance(rag_meta, dict):
            rag_meta["status"] = "pending"
        try:
            embed_start = time.perf_counter()
            vector = await embed_query(query)
            embed_latency = int((time.perf_counter() - embed_start) * 1000)
            if isinstance(rag_meta, dict):
                rag_meta["embedding_latency_ms"] = embed_latency
            ...
            if isinstance(rag_meta, dict):
                rag_meta["search_latency_ms"] = search_latency

            matches = resp.get("matches", [])
            if not matches:
                if isinstance(rag_meta, dict):
                    rag_meta["status"] = "miss"
                    rag_meta["match_count"] = 0
                emit_metric(
                    "rag_retrieval",
                    status="miss",
                    agent_id=agent_id,
            ...
            if isinstance(rag_meta, dict):
                rag_meta["status"] = "hit"
                rag_meta["match_count"] = len(parts)
